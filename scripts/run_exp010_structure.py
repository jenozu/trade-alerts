...
from structure_event_research import (
    analyze,
    enrich,
    markdown_report,
    metrics_frame,
...
def main():
    ...
    for year in (2023, 2024, 2025):
        enriched[year] = enrich(
            pd.read_csv(ledgers[year]),
            pd.read_parquet(features[year]),
        )

    result, classified = analyze(enriched)

    out = Path(args.output_dir)
    out.mkdir(parents=True, exist_ok=True)

    (out / "EXP-010-Structure-Shift.md").write_text(
        markdown_report(result),
        encoding="utf-8",
    )

    (out / "exp010_structure_results.json").write_text(
    ...
    classified.to_csv(
        out / "classified_structure_trades.csv",
        index=False,
    )
