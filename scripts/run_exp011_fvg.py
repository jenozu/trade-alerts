...
def main():
    ...
    (out / "EXP-011-FVG-IFVG.md").write_text(
        markdown_report(result),
        encoding="utf-8",
    )

    (out / "exp011_fvg_ifvg_results.json").write_text(
        json.dumps(
            result,
            indent=2,
            sort_keys=True,
    ...
    metrics_frame(result).to_csv(
        out / "fvg_ifvg_metrics.csv",
        index=False,
    )

    classified.to_csv(
        out / "classified_fvg_trades.csv",
        index=False,
    )
