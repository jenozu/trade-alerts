...
def main():
    ...
    (out / "EXP-018-SR-Confluence.md").write_text(
        markdown_report(result),
        encoding="utf-8",
    )

    (out / "exp018_sr_confluence_results.json").write_text(
        json.dumps(
            result,
            indent=2,
            sort_keys=True,
    ...
    metrics_frame(result).to_csv(
        out / "sr_confluence_metrics.csv",
        index=False,
    )

    classified.to_csv(
        out / "classified_sr_confluence_trades.csv",
        index=False,
    )
