...
from volume_rvol_research import (
    analyze,
    enrich,
    markdown_report,
    metrics_frame,
...
def main():
    ...
    (out / "EXP-015-Volume-RVOL.md").write_text(
        markdown_report(result),
        encoding="utf-8",
    )

    (out / "exp015_volume_rvol_results.json").write_text(
        json.dumps(
            result,
            indent=2,
            sort_keys=True,
    ...
    metrics_frame(result).to_csv(
        out / "volume_rvol_metrics.csv",
        index=False,
    )

    classified.to_csv(
        out / "classified_volume_trades.csv",
        index=False,
    )
