...
from premium_discount_research import (
    analyze,
    enrich,
    markdown_report,
    metrics_frame,
...
def main():
    ...
    (out / "EXP-013-Premium-Discount.md").write_text(
        markdown_report(result),
        encoding="utf-8",
    )

    (out / "exp013_premium_discount_results.json").write_text(
        json.dumps(
            result,
            indent=2,
            sort_keys=True,
    ...
    metrics_frame(result).to_csv(
        out / "premium_discount_metrics.csv",
        index=False,
    )

    classified.to_csv(
        out / "classified_premium_discount_trades.csv",
        index=False,
    )
